...
class GetMessagesTestCase(unittest.TestCase):
    ...
    @patch('furious.batcher.MessageIterator', autospec=True)
    def test_no_messages(self, iterator):
        """Ensure that no payloads are returned if no messages are found."""
        from furious.batcher import get_messages

        iterator.return_value = iter([])

        result = get_messages('t', 'qn', 1)
    ...
    @patch('furious.batcher.MessageIterator', autospec=True)
    def test_has_a_single_message(self, iterator):
        """Ensure that a single payload is returned if one message is found."""
        from furious.batcher import get_messages

        payload = ["payload"]

        iterator.return_value = iter([payload])
        ...
        self.assertEqual([payload], list(result))

    @patch('furious.batcher.MessageIterator', autospec=True)
    def test_has_a_multiple_messages(self, iterator):
        """Ensure that no messages are returned if none are found."""
        from furious.batcher import get_messages

        payload = ["payload"]
        payload1 = ["payload"]

        iterator.return_value = iter([payload, payload1])
        ...
        self.assertEqual([payload, payload1], list(result))
